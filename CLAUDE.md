...
- `types.ts` — domain types (`GameState`, `City`, `Store`, `CompanyPolicy`, archetypes, terrain, etc.) and constants like `MAX_STORES`.
...
- `mapRender.ts` — converts a `GameState` (or a bare starter map) into a `CityMapSnapshot` for the renderer. Tests live as `*.spec.ts` next to each module.
...
### Map rendering: Phaser scene driven by snapshots

`src/lib/components/game/CityMap.svelte` mounts a Phaser game whose scene is `src/lib/phaser/cityMapScene.ts`. The scene is **snapshot-driven**: it never reaches into game state, only renders the latest `CityMapSnapshot` and emits `tileSelected` events back to the Svelte component. Tile size is `32` px (used by both the scene and e2e tile-click math).
