...
- `city.ts` — city/tile generation and placement validation (`generateCity`, `getTileById`, `getTilePlacementBlockReason`). `getNeighborhood` lays out districts geographically by tile position (`downtown`, `campus`, `mall`, `residential`, `suburb`, `parkEdge`, `transit`); each `NeighborhoodId` has a `NEIGHBORHOOD_PROFILES` entry that seeds per-tile `demand`/`rent`/`footTraffic`/`customerFit` (jittered by the seeded RNG), and `rent` is what drives a tile's land/setup cost. `getTilesByNeighborhood` filters tiles by district.
...
- `state.ts` — policy and decision transitions (`DEFAULT_POLICY`, `updatePolicy`, `resolveDecision`), plus store **land/setup cost** via `getExpansionSetupCost`: it scales with the tile's district `rent` and adds a per-terrain `TERRAIN_SETUP_COST_PREMIUM` (commercial > residential), so district choice drives both opening cost and projected daily rent.
...
- `industry.ts`, `industryPlacement.ts`, `industryProduction.ts` — parallel industry simulation: deterministic district-based `IndustryCity`, raw resources (farmland/forest/mine anchors), `MaterialDefinition` + `ProductionRecipe`, industrial building placement, and daily production chain that feeds warehouses consumed by `stock.ts`. Building types carry a `tier` (1–3) used only for build-menu grouping and chain badges — Tier 1 chains (bottled water, produce, pantry) are the 2–3 building onboarding chains.
...
- `productChainGraph.ts` — warehouse-flow graph (material → recipe → warehouse nodes with `ProductChainHealth`) linking industry output to retail demand, plus the shared metric/health helpers used by both graph builders.
- `productChainTree.ts` — tree-per-product chain view builder: duplicates shared sub-chains per branch (path-suffixed node ids) for a crossing-free layout; also owns `buildStoreCategoryChainSummaries`. `productChainGraph.ts` retains the warehouse-flow graph and the shared metric/health helpers.
...
- **Image assets must be created with image-generation tools, not drawing/generation scripts.** Do not create terrain, road, river, building, storefront, icon, or other game art by scripting pixels/shapes with Node/Python/canvas/PNG libraries. Use the available image-generation/editing workflow for raster art, then register and test the finished files in `gameArt.ts`.
- Terrain feature sprite orientation is asset-specific: `road-tile.png` is natively horizontal, while `river-tile.png` is natively vertical. Keep `cityMapScene.ts` rotation logic feature-aware instead of sharing one road/river rotation rule. Complex road/river connectors are registered through `ROAD_TERRAIN_CONNECTOR_ART` / `RIVER_TERRAIN_CONNECTOR_ART`; do not reintroduce the legacy `roadIntersection` art id or `road-intersection-tile.png` path. Connector variants should be derived from the same straight road/river palette and linework so corners, tees, and intersections match the base tiles.
- Retail cities should not generate `industrial` terrain; industrial terrain belongs to `IndustryCity`. Retail road grids should divide districts with connected row/column dividers while leaving river tiles as river so both networks remain contiguous.
- Residential terrain uses `RESIDENTIAL_TERRAIN_ART_VARIANTS`; keep variants as road-free residential lots that mix single-home tiles with yard/garden/open-space tiles so residential districts do not render as one house on every grid square. Short paths, fences, gardens, and trees are fine. Do not embed road/street strips in residential terrain art, because road orientation belongs to the map feature layer.
- Terrain art is **neighborhood-aware** for commercial districts: `cityMapScene.ts` looks up `NEIGHBORHOOD_TERRAIN_ART[tile.neighborhood]` (downtown/campus/mall variant sets) before falling back to the plain `commercial`/`green` art, so the per-tile `neighborhood` field must flow through `mapRender.ts` into the `CityMapSnapshot`. Register new district variant sets in `gameArt.ts` (they are wired into `TERRAIN_ART_LIST` for the `gameArt.spec.ts` completeness check) and pick variants deterministically per tile, never randomly per frame.
- The canvas exposes `data-*` attributes (`data-store-sprite-count`, `data-terrain-asset-mode`, `data-terrain-base-sprite-count`, etc.) that e2e tests read to assert the renderer settled — see `src/routes/retail-sim.e2e.ts` for expected counts (e.g. base sprites = `width * height` = 2688 on the 56×48 starter city). Update those asserts when changing terrain generation.
