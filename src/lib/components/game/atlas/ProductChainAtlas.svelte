<script lang="ts">
	...
	const yStep = $derived(compact ? 112 : 150);
	const xPad = $derived(compact ? 30 : 40);
	const yPad = $derived(compact ? 30 : 36);
	...
	const layout = $derived.by(() => {
		...
		return {
			...
			height: Math.max(maxY + yPad, compact ? 280 : 330)
		};
	});
...
<section class={['product-chain-atlas', compact && 'is-compact']} aria-label={graph.title}>
	...
	{:else}
		<div class="atlas-body" class:single={compact || !broadside}>
			<ChainMap {i18n} width={layout.width} height={layout.height} {compact}>
				<div
					class="canvas-inner"
					role="presentation"
			...
			{#if !compact && broadside}
				<aside class="broadside-slot">{@render broadside()}</aside>
			{/if}
		</div>
