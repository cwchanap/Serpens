...
function evaluation(): ScenarioRun['evaluation'] {
	return {
		...
		risks: [
			{ kind: 'condition', conditionId: 'share', distance: 0.1, triggered: true }
		],
		projection: { score: 650, medal: 'bronze', componentPoints: [], componentEvidence: [] }
	};
}

describe('scenario copy branch coverage', () => {
	it('formats currency, percent, decimal, all window kinds, triggered risks, and deadlines', () => {
	...
	] as const)('formats %s result score %s across medal branches', (outcome, score, medal, bestUpdated, nextMedal) => {
		const current = definition();
	...
	it('maps every share-code and operation error code to localized copy', () => {
		...
		expect(shareCodes.map((code) => scenarioShareCodeErrorText(code, i18n))).toSatisfy(
			(values: string[]) => values.every(Boolean)
		);
		expect(
			operationCodes.map((code) =>
				scenarioDiagnosticText({ code, diagnostics: [] } as ScenarioOperationError, i18n)
			)
		).toSatisfy((values: string[]) => values.every(Boolean));
	});
});
